...
namespace server
{
    ...
    VAR  (Verbose         , 0, 1, 1); // Should the server be verbose or no?
    ...
    VAR  (MaxReservedTags , 1, 2, 3); // Maximal amount of reserved clantags per account.
    VAR  (MaxMembers,     5, 15, 25); // Maximal amount of members for each clan.
    ...
    VARP(showAuthDesc, 0, 1, 1);

    bool Verify (clientinfo *, const char *);
    ...
    {
        if(ci->wrong_attempts >= MaxAttempts) {
            if (ci->wrong_attempts == MaxAttempts)
            {
                defformatstring (Message)("\f3%s \f4Maximal amount of fails reached, ignoring /setmaster, #register and #reserve... \f3%s", MessageDecoration1, MessageDecoration2);
        ...
        if(val && authname) 
        {
            if(authdesc && authdesc[0] && showAuthDesc) formatstring(msg)("%s claimed %s as '\fs\f5%s\fr' [\fs\f0%s\fr]", colorname(ci), name, authname, authdesc);
    ...
    int AddAccountStats (clientinfo *, uint, uint, double, double, uint, uint);
    void SetRanking ();
    ...
    void checkintermission()
    {
        if(gamemillis >= gamelimit && !interm)
        {
            ...
            loopv (clients)
            {
                if (clients [i]->acc && clients [i]->verified)
                {
                    AddAccountStats (clients [i], clients [i]->state.frags, clients [i]->state.deaths, 
                        ((float) clients [i]->state.frags  / max (clients [i]->state.deaths, 1)),
                        (clients [i]->state.damage * 100 / max (clients [i]->state.shotdamage, 1)),
                        m_teammode ? clients [i]->state.teamkills : 0,
                        (m_ctf || m_collect) ? clients [i]->state.flags : 0
                    );
                    SetRanking ();
                }
            }
    ...
    // >>> HaythServ-v2.0 Core
        ...
        // Explode function by zeromod, see zeromod argsep function
            inline void Explode (char * String, char * Array [], int Count = 2, char Separator = ' ')
                { char * Current; for (int i = 1; i < Count; i++) Array [i] = 0; Array [0] = String; if (!String || !*String) return; for (int i = 1; i < Count; i++) { Current = strchr (Array [i - 1], Separator); if (!Current) return; *Current = 0; Current ++; while (*Current == Separator) Current++; Array [i] = Current; } }
        ...
        // >>> Flagruns core
            ...
            vector <flagrun> flagruns;

            void checkflagrun (clientinfo * ci, int flagruntime)
            {
                ...
                else
                {
                    bool isbest = false;
                    flagrun * fr = 0;
                    loopv (flagruns) if (!strcmp (smapname, flagruns[i].Map) && gamemode == flagruns[i].Mode) {fr = &flagruns[i]; break;}
                    if (!fr) // No flagrun found for this map and mode -> best flagrun in this map and mode
                    {
                        flagrun & curfr = flagruns.add ();
                        copystring (curfr.Name, ci->name);
                        curfr.Millis = flagruntime;
                        copystring (curfr.Map, smapname);
                        curfr.Mode = gamemode;
                        isbest = true;
                    }
                    else
                    {
                        if (fr->Millis >= flagruntime)
                        {
                            copystring (fr->Name, ci->name);
                            fr->Millis = flagruntime;
                            isbest = true;
                        }
                        else
                        {
                    ...
                    if (isbest)
                    {
                        stream *f = openutf8file(frunsfile, "w");
                        if(f)
                        {
                            f->printf("// Automatically generated by HaythServ-v2.0 - do not edit.\n\n");
                            loopv (flagruns)
                            {
                                defformatstring (Message)("flagrun %s %i %s %i\n", 
                                    flagruns[i].Name, flagruns[i].Millis, flagruns[i].Map, flagruns[i].Mode
                                );
                                f->printf (Message);
                            }
                            delete f;
            ...
            void NewFlagrun (const char * Name, int Millis, const char * Map, int Mode)
            {
                flagrun & fr = flagruns.add ();
                copystring (fr.Name, Name);
                fr.Millis = Millis;
                copystring (fr.Map, Map);
                fr.Mode = Mode;
            }

            ICOMMAND (flagrun, "sisi", (const char * n, int * m, const char * M, int * Mo), NewFlagrun (n, * m, M, * Mo));
        ...
        // >>> Accounting system core
            ...
            vector<ReservedTag> ReservedTags;

            bool Verify (clientinfo * ci, const char * Hash)
            {
                string User_n_Pass;
                loopv (accounts)
                {
                    formatstring (User_n_Pass)("%s:%s", accounts[i].username, accounts[i].password);
                    ...
                    if (OK)
                    {
                        ...
                        {
                            sendservmsgf("%s verified as '\fs\f5%s\fr'", 
                                colorname (ci),
                                accounts[i].username
                            );
                            return true;
            ...
            int AddAccountStats (clientinfo * ci, uint frags, uint deaths, double KPD, double Acc, uint TKs, uint Scored)
            {
                if (!ci->acc || !ci->verified) return 1;
                ci->acc->global_frags += frags;
                ci->acc->global_deaths += deaths;
                ci->acc->global_kpd += KPD;
                ci->acc->global_accuracy += Acc;
                ci->acc->global_teamkills += TKs;
                ci->acc->global_scored += Scored;
                ci->acc->global_matches += 1;
                stream *f = openutf8file(loginfile, "w");
                if(f)
                {
                    f->printf("// Automatically generated by HaythServ-v2.0 - do not edit, add accounts ingame by #register <username> <password>\n");
                    f->printf("// Change accounts privileges ingame as root by #setpriv <username> <privileges>, reserve names ingame by #reserve [username].\n\n");
                    string Message;
                    loopv (accounts)
                    {
                        formatstring (Message)("account %s %s %s %i %i\n",  
                            accounts[i].username, accounts[i].password, accounts[i].ipaddr, accounts[i].hidden, accounts[i].privileges
                        );
                        f->printf (Message);
                        formatstring (Message)("stats %s %u %u %f %u %u %i\n",
                            accounts[i].username, accounts[i].global_frags, accounts[i].global_deaths,
                            accounts[i].global_accuracy, accounts[i].global_teamkills, accounts[i].global_scored,
                            accounts[i].global_matches
                        );
                        f->printf (Message);
                    }
                    loopv (ReservedNames)
                    {
                        formatstring (Message)("reserve %s %s\n", 
                            ReservedNames[i].acc->username, ReservedNames[i].name
                        );
                        f->printf (Message);
                    }
                    loopv (ReservedTags)
            ...
            void SetRanking ()
            {
                ...
                do
                {
                    ...
                    loopv (accounts)
                    {
                        account * nextacc = &accounts[i+1];
                        if (nextacc)
                        {
                            double score1 = accounts[i].global_frags - accounts[i].global_deaths - (accounts[i].global_teamkills * 2) + accounts[i].global_accuracy;
                            double score2 = accounts[i+1].global_frags - accounts[i+1].global_deaths - (accounts[i+1].global_teamkills * 2) + accounts[i+1].global_accuracy;
                            if (score1 < score2)
                            {
                                /* Swap positions, 'cause the next account has a better ranking than this one. */
                ...
                loopv (accounts)
                {
                    accounts[i].global_ranking = i+1; /* Sets the ranking */
                }
            }

            void Register (clientinfo * ci, const char * username, const char * password)
            {
                if (ci->wrong_attempts >= MaxAttempts)
                {
                    if (ci->wrong_attempts == MaxAttempts)
                    {
                        defformatstring (Message)("\f3%s \f4Maximal amount of fails reached, ignoring #register, #reserve and /setmaster... \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                if (ci->verified)
                {
                    defformatstring (Message)("\f3%s \f4You have already verified. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                const char * ipaddr = getclienthostname (ci->clientnum);
                int Counter = 0;
                loopv(accounts)
                {
                    if (!strcmp (accounts[i].username, username))
                    {
                        defformatstring (Message)("\f3%s \f4An account with the username \"\f0%s\f4\" \f4does already exist. \f3%s",
                            MessageDecoration1,
                            username,
                            MessageDecoration2
                        );
                    ...
                    if (accounts[i].ipaddr == ipaddr)
                    {
                        Counter++;
                    }
                }
                if (Counter >= MaxAccountsPerIP)
                {
                    defformatstring (Message)("\f3%s \f4We're sorry, but you reached the limit of accounts per IP. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                account & acc = accounts.add ();
                copystring (acc.username, username);
                copystring (acc.password, password);
                copystring (acc.ipaddr, ipaddr);
                acc.privileges = 0;
                acc.hidden = 0;
                ...
                defformatstring (Message)("\f3%s \f4Your account has been succsessfully created. \f3%s",
                    MessageDecoration1,
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void Reserve (clientinfo * ci, const char * name)
            {
                if (ci->wrong_attempts >= MaxAttempts)
                {
                    if (ci->wrong_attempts == MaxAttempts)
                    {
                        defformatstring (Message)("\f3%s \f4Maximal amount of fails reached, ignoring #reserve, #register and /setmaster... \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                if (!ci->verified || !ci->acc)
                {
                    defformatstring (Message)("\f3%s \f4You need to verify to use this command. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                {
                    defformatstring (Message)("\f3%s \f4You can't reserve this name. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                loopv (ReservedNames)
                {
                    if (!strcmp (ReservedNames[i].name, name))
                    {
                        defformatstring (Message)("\f3%s \f4This account is already reserved \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                    ...
                    Cur = ReservedNames[i].acc;
                    if (Cur == ci->acc)
                    {
                        Counter++;
                    }
                }
                if (Counter >= MaxReservedNames)
                {
                    defformatstring (Message)("\f3%s \f4We're sorry - but you reached the maximal amount of reserved names per account. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                ReservedName & resname = ReservedNames.add ();
                copystring (resname.name, name);
                resname.acc = ci->acc;
                stream *f = openutf8file(loginfile, "w");
                if(f)
                {
                ...
                defformatstring (Message)("\f3%s \f4This username has been reserved \f0succsessfully\f4. \f3%s",
                    MessageDecoration1,
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void ChangePrivs (clientinfo * ci, const char * username, const char * privs)
            {
                string Message;
                bool found = false;
                int newprivs;
                if (!strcmp (privs, "master") || !strcmp (privs, "1"))
                {
                    newprivs = PRIV_MASTER;
                }
                else if (!strcmp (privs, "auth") || !strcmp (privs, "2"))
                {
                    newprivs = PRIV_AUTH;
                }
                else if (!strcmp (privs, "admin") || !strcmp (privs, "3"))
                {
                    newprivs = PRIV_ADMIN;
                }
                else if (!strcmp (privs, "root") || !strcmp (privs, "4"))
                {
                    newprivs = PRIV_ROOT;
                }
                else
                ...
                loopv (accounts)
                {
                    ...
                    {
                        found = true;
                        if (accounts[i].privileges != newprivs)
                        {
                            formatstring (Message)("\f3%s \f4Succsessfully changed \f6%s\f4's privilege level to \f6%i\f4. \f3%s",
                                MessageDecoration1,
                                accounts[i].username,
                                newprivs,
                                MessageDecoration2
                            );
                            accounts[i].privileges = newprivs;
                        }
                        else
                        {
                ...
                if (!found)
                {
                    formatstring (Message)("\f3%s \f4Cannot change \f6%s\f4's privilege level: \f6No such account. \f3%s",
                        MessageDecoration1,
                        username,
                        MessageDecoration2
                    );
            ...
            void Rename (clientinfo * ci, bool IsClanTag = false)
            {
                const char * NewName = "unnamed";
                ...
                defformatstring (Message)("\f3%s \f4You need to verify to use this %s, verify now using \f6/setmaster \f0username\f4:\f0password \f3%s",
                    MessageDecoration1,
                    IsClanTag ? "\f3clan tag\f4" : "\f0name\f4",
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ci->wrong_names++;
            ...
            void NewAccount (const char * username, const char * password, const char * IP, int hidden, int privileges)
            {
                int Counter = 0;
                loopv (accounts)
                {
                    if (!strcmp (accounts[i].ipaddr, IP))
                        Counter++;
                }
                if (Counter > MaxAccountsPerIP)
                {
                    if (Verbose) { logoutf ("Ignoring new account creation: Accounts limit reached for IP %s", IP); }
                    return;
                }
                account & acc = accounts.add();
                copystring (acc.username, username);
            ...
            void NewReserve (const char * user, const char * name)
            {
                ...
                if (!acc) { if (Verbose) { logoutf ("Ignoring new name reserving: Unknown username %s", user); } return; }
                loopv (ReservedNames)
                {
                    if (ReservedNames[i].acc == acc)
                    {
                        Counter++;
                    }
                }
                if (Counter >= MaxReservedNames) { if (Verbose) { logoutf ("Ignoring new name reserving: Reserved names limit reached for account %s", user); } return; }
                ReservedName & resname = ReservedNames.add ();
                copystring (resname.name, name);
                resname.acc = acc;
            }

            void NewTagReserve (const char * O, const char * CT, const char * CN)
            {
                int Counter = 0;
                account * acc = 0;
                loopv (accounts)
                {
                    if (!strcmp (accounts[i].username, O))
                    {
                        acc = &accounts[i];
                        break;
                    }
                }
                if (!acc) { if (Verbose) { logoutf ("Ignoring new clantag reserve: No such username: %s", O); } return; }
                loopv (ReservedTags)
                {
                    if (ReservedTags[i].owner == acc)
                    {
                        Counter++;
                    }
                }
                if (Counter >= MaxReservedTags) { if (Verbose) { logoutf ("Ignoring new clantag reserve: Clantags limit reached for account %s", O); } return; }
                ReservedTag & resclan = ReservedTags.add ();
                copystring (resclan.clanname, CN);
                copystring (resclan.clantag , CT);
                resclan.owner = acc;
            }

            void NewAuthorized (const char * CT, const char * U)
            {
                int Counter = 0;
                account * acc = 0;
                ReservedTag * Clan = 0;
                loopv (accounts)
                {
                    if (!strcmp (accounts[i].username, U))
                    {
                        acc = &accounts[i];
                        break;
                    }
                }
                if (!acc) { if (Verbose) { logoutf ("Ignoring new clantag authorized user: No such username: %s", U); } return; }
                loopv (ReservedTags)
                {
                    if (!strcmp (ReservedTags[i].clantag, CT))
                    {
                        Clan = &ReservedTags[i];
                        break;
                    }
            ...
            void NewStats (const char * user, uint frags, uint deaths, double Acc, uint TKs, uint Scored, uint Matches)
            {
                ...
                if (!acc) { if (Verbose) { logoutf ("Ignoring global statistics: No such username: %s", user); } return; }
                acc->global_frags = frags;
                acc->global_deaths = deaths;
                acc->global_kpd = ((float) frags  / max ((int) deaths, 1));
                acc->global_accuracy = Acc;
                acc->global_teamkills = TKs;
                acc->global_scored = Scored;
        ...
        // >>> Commands system core
            ...
            int ParseCommand (clientinfo * ci, const char * Cmd)
            {
                if (!Cmd || Cmd == NULL)
                    return 1;
                Command * Current = 0;
                ...
                loopv (Commands)
                {
                    if (!strcmp (Commands[i].Name, (const char *)Array [0]))
                    {
                        Current = &Commands[i];
                    }
                }
                ...
                if (!Current)
                {
                    formatstring (Message) ("\f3%s \f4Unknown command: \f6%s \f3%s",
                        MessageDecoration1,
                        (const char *) Array [0],
                        MessageDecoration2
                    );
                ...
                if (Current->RequiredPrivileges > ci->privilege)
                {
                    formatstring (Message)("\f3%s \f4Permission denied. \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                Current->Function(ci, Array [1] ? Array [1] : "");
                logoutf ("Player command from %s: %s%s%s", ci->name, Current->Name, Array [1] ? " with args: " : "", Array [1] ? Array [1] : "");
                return 0;
            }
            ...
            void Help (clientinfo * ci, const char * Args)
            {
                ...
                loopv (Commands)
                {
                    ...
                    {
                        if (Commands[i].RequiredPrivileges > ci->privilege)
                        {
                            formatstring (Message)("\f3%s \f0Help: \f3Permission denied\f4. \f3%s",
                                MessageDecoration1,
                                MessageDecoration2
                            );
                            sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                loopv (Manpages)
                {
                    if (!strcmp (Manpages[i].Command, Cmd))
                    {
                        formatstring (Message)("\f3%s \f0Help: \f3#\f4%s \f0%s\f4: \f0%s \f3%s",
                            MessageDecoration1,
                            Cmd,
                            Manpages[i].Arguments,
                            Manpages[i].Manpage,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                loopv (Commands)
                {
                    if (!strcmp (Commands[i].Name, Cmd))
                    {
                        formatstring (Message)("\f3%s \f4HELP: \f6No manpage for command %s found. \f3%s",
                            MessageDecoration1,
                            Cmd,
                            MessageDecoration2
                        );
                ...
                formatstring (Message)("\f3%s \f4HELP: \f6Unknown command: %s\f4. \f3%s", 
                    MessageDecoration1,
                    Cmd,
                    MessageDecoration2
                );
            ...
            int NewManpage (const char * Command, const char * Arguments, const char * Manpage)
            {
                loopv (Manpages)
                    if (!strcmp (Manpages[i].Arguments, Arguments)) { return 1; }
                uint Current = Manpages.length ();
                Manpages.add ();
                copystring (Manpages[Current].Command, Command);
                copystring (Manpages[Current].Arguments, Arguments);
                copystring (Manpages[Current].Manpage, Manpage);
                return 0;
            }

        // <<< Man system core

        // >>> Teams and bots persisting core

            void Persist (int Type, int Value)
            {
                switch (Type)
                {
                    case 1:
        ...
        // >>> Commands declaration
            ...
            void _HELP (clientinfo * ci, const char * Args)
            {
            ...
            void _LOGIN (clientinfo * ci, const char * Args)
            {
                defformatstring (Message)("\f3%s \f4Usage: \f6/setmaster \f0username\f4:\f0password \f3%s",
                    MessageDecoration1,
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _REGISTER (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    Explode ((char *) Args, Array, 3, ' ');
                    if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                        Register (ci, Array [0], Array [1]);
                    else
                    {
                        defformatstring (Message)("\f3%s \f4Usage: \f6#register username password \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _RESERVE (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
            ...
            void _SETPRIVS (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    ...
                    if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                    {
                        ChangePrivs (ci, Array [0], Array [1]);
                    }
                    else
                    {
                        defformatstring (Message)("\f3%s \f4Usage: \f6#setprivs username newprivs \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _GIVEMASTER (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    Explode ((char *) Args, Array, 128, ',');
                    for (int i = 0; i < 128; i ++)
                    {
                        if (Array[i] && Array[i] != 0 && Array[i] != NULL)
                        {
                            ...
                            {
                                if (cx->privilege > ci->privilege)
                                {
                                    defformatstring (Message)("\f3%s \f4Cannot change %s's privileges: \f0Permission denied. \f3%s",
                                        MessageDecoration1,
                                        colorname (cx),
                                        MessageDecoration2
                                    );
                ...
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#givemaster cn[,cn2[,...]] \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _GIVEADMIN (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                ...
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#giveadmin cn[,cn2[,...]] \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _INFO (clientinfo * ci, const char * Args)
            {
                ...
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                formatstring (Message)("\f3%s \f4Thanks to \f6/dev/zero \f4for his \f6/setmaster \f0username\f4:\f0password \f4idea. \f3%s",
                    MessageDecoration1,
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                ...
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                formatstring (Message)("\f3%s \f4Running on a %s %s \f4machine. \f3%s",
                    MessageDecoration1,
                    (sizeof (void *) == 8) ? "\f364\f4-bit" : "\f132\f4-bit",
// Zeromod os detection
...
# endif
            ...
            void _RENAME (clientinfo * ci, const char * Args)
            {
                char * Array [4096];
                Explode ((char *) Args, Array, 2, ' ');
                if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                {
                    ...
                    loopv (clients)
                    {
                        if (clients[i]->clientnum == cn) found = true;
                    }
                    if (!found)
                    {
                        defformatstring (Message)("\f3%s \f4Unknown client number. \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                    ...
                    if (cx->privilege > ci->privilege)
                    {
                        defformatstring (Message)("\f3%s \f4Permission denied. \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                    ...
                    putint (p, b.len);
                    p.put (buf, b.len);
                    sendpacket (cx->ownernum, 1, p.finalize());
                    cx->wrong_names++;
                }
                else
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#rename <cn> <newname> \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _MSG (clientinfo * ci, const char * Args)
            {
                char * Array [4096];
                Explode ((char *) Args, Array, 2, ' ');
                if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                {
                    ...
                    for (int i = 0; i < 128; i++)
                    {
                        if (cns[i] && cns[i] != NULL)
                        {
                            ...
                            {
                                defformatstring (Message)("\f3%s \f4PM from \f5%s\f4(\f1%i\f4): \f0%s \f3%s",
                                    MessageDecoration1,
                                    ci ? colorname (ci) : "",
                                    ci ? ci->clientnum  : -1,
                                    Array[1],
                                ...
                                sendf (cx ? cx->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                            }
                        }
                    }
                }
                ...
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#pm <cn[,cn2[,...]]> <message> \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _REVOKEPRIV (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    ...
                    for (int i = 0; i < 128; i ++)
                    {
                        if (Array[i] && Array[i] != 0 && Array[i] != NULL)
                        {
                            ...
                            else
                            {
                                if (cx->privilege > ci->privilege)
                                {
                                    defformatstring (Message)("\f3%s \f4Cannot revoke %s's privileges: \f0Permission denied. \f3%s",
                                        MessageDecoration1,
                                        colorname (cx),
                                        MessageDecoration2
                                    );
                ...
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#revokepriv cn[,cn2[,...]] \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _STATS (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Cns [4096];
                    Explode ((char *) Args, Cns, 128, ',');
                    for (int i = 0; i < 128; i++)
                    {
                        if (Cns [i] && Cns [i] != NULL)
                        {
                            ...
                            else
                            {
                                string Message;
                                string Bak;
                                formatstring (Message)("\f3%s \f0Current game statistics for \f6%s\f4(\f1%i\f4): Frags: \f0%i\f4, deaths: \f0%i\f4 (KPD: \f0%.2f\f4), accuracy: \f0%.2f%%",
                                    MessageDecoration1,
                                    cx->name,
                                    cx->clientnum,
                                    cx->state.frags,
                                ...
                                if (m_teammode)
                                {
                                    formatstring (Message)("%s, \f4team kills: \f0%i",
                                        Bak,
                                        cx->state.teamkills
                                    );
                                }
                                ...
                                if (m_ctf || m_collect)
                                {
                                    formatstring (Message)("%s, \f4scored: \f0%i",
                                        Bak,
                                        cx->state.flags
                                    );
                                }
                                ...
                                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                                if (cx->verified && cx->acc)
                                {
                                    formatstring (Message)("\f3%s \f0Global statistics for \f6%s\f4(\f1%i\f4): Rank: \f0%u\f4 frags: \f0%u\f4, deaths: \f0%u\f4 (KPD: \f0%.2f\f4), accuracy: \f0%.2f%%\f4, team kills: \f0%u\f4, scored: \f0%u\f4, total games: \f0%u \f3%s",
                                        MessageDecoration1,
                                        cx->name,
                                        cx->clientnum,
                                        cx->acc->global_ranking,
                                        cx->acc->global_frags,
                                        cx->acc->global_deaths,
                                        cx->acc->global_kpd,
                                        cx->acc->global_accuracy,
                                        cx->acc->global_teamkills,
                                        cx->acc->global_scored,
                                        ci->acc->global_matches,
                                        MessageDecoration2
                                    );
                                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                                }
                ...
                else
                {
                    ...
                    if (m_teammode)
                    {
                        formatstring (Message)("%s, \f4teamkills: \f0%i",
                            Bak,
                            ci->state.teamkills
                        );
                    }
            ...
            void _PERSIST (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    Explode ((char *) Args, Array, 2, ' ');
                    if (Array[0] && Array[0] != NULL)
                        Persist (1, atoi (Array [0]));
                    else
                        Persist (1, PersistTeams ? 0 : 1);
                }
                else
                {
                    Persist (1, PersistTeams ? 0 : 1);
            ...
            void _NOCLEARBOTS (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    Explode ((char *) Args, Array, 2, ' ');
                    if (Array[0] && Array[0] != NULL)
                        Persist (2, atoi (Array [0]));
                    else
                        Persist (2, PersistTeams ? 0 : 1);
                }
                else
                {
                    Persist (2, PersistTeams ? 0 : 1);
            ...
            void _EXEC (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    execute (Args);
                    defformatstring (Message)("\f3%s \f4Command runned succsessfully: \f0%s \f3%s",
                        MessageDecoration1,
                        Args,
                        MessageDecoration2
                    );
                ...
                {
                    defformatstring (Message)("\f3%s \f4Usage: \f6#exec command \f3%s",
                        MessageDecoration1,
                        MessageDecoration2
                    );
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _PBAN (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    ...
                    if (Array[0] && Array[0] != NULL)
                    {
                        ...
                        if (!cx)
                        {
                            defformatstring (Message)("\f3%s \f4Unknown client number: \f0%i \f3%s",
                                MessageDecoration1,
                                atoi (Array [0]),
                                MessageDecoration2
                            );
                        ...
                        const char * Name = cx->name;
                        int Cn = cx->clientnum;
                        Cheater & PBan = Cheaters.add ();
                        PBan.IP = getclientip (cx->clientnum);
                        sendservmsgf ("\f3%s \f4Client \f0%s\f4(\f0%is\f4) has been disconnected because: Client added to the list of the permamently banned clients. \f3%s",
                            MessageDecoration1,
                            Name,
                            Cn,
                            MessageDecoration2
                    ...
                    {
                        defformatstring (Message)("\f3%s \f4Usage: \f6#pban CN \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void _GETIP (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    char * Array [4096];
                    Explode ((char *) Args, Array);
                    if (Array [0] && Array [0] != NULL)
                    {
                        clientinfo * cx = getinfo (atoi (Array [0]));
                        if (!cx)
                        ...
                        if (cx->clientnum >= 128)
                        {
                            defformatstring (Message)("\f3%s \f4Client is a bot - Cannot get a bot's IP. \f3%s",
                                MessageDecoration1,
                                MessageDecoration2
                            );
                            sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                        ...
                        defformatstring (Message)("\f3%s \f0%s \f4(\f0%i\f4)'s IP-Address: \f0%s \f3%s",
                            MessageDecoration1,
                            cx->name,
                            cx->clientnum,
                            getclienthostname (cx->clientnum),
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                    }
                    ...
                    {
                        defformatstring (Message)("\f3%s \f4Usage: \f6#getip CN \f3%s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
        ...
        // >>> Commands / manpages initialization
            ...
            void InitializeManpages ()
            {
                NewManpage ("help", "(Command name)", "Displays a list of available commands, or displays help for a specific command.");
                NewManpage ("man", "(Command name)", "Displays a list of available commands, or displays help for a specific command.");
                NewManpage ("info", "", "Displays information about servermod and server.");
                NewManpage ("login", "", "Use \fs\f6/setmaster \f0username\f4:\f0password\fr to verify instead.");
                NewManpage ("reserve", "(Name)", "Reserves the current name, or another one if specified.");
                NewManpage ("register", "<Username> <Password>", "Creates an account, may be used for name protection.");
                NewManpage ("givemaster", "<cn(,cn2(,...))>", "Gives master to a client, may be used for multiple clients (#givemaster cn1,cn2,...).");
                NewManpage ("giveadmin", "<cn(,cn2(,...))>", "Gives master to a client, may be used for multiple clients (#giveadmin cn1,cn2,...).");
                NewManpage ("rename", "<cn> <New name>", "Renames a client.");
                NewManpage ("setprivs", "<account> <newprivs>", "Changes privilege level for an account (supports 0/1/2/3/4 or none/master/admin/root.)");
                NewManpage ("revokepriv", "<cn(,cn2(,...))>", "Revokes a client's privileges");
                NewManpage ("pm", "<cn(,cn2(,...))> <message>", "Sends a private message to a single or multiple clients.");
                NewManpage ("stats", "(cn(,cn2(,...)))", "Displays your current stats, or another player's stats.");
                NewManpage ("persist", "(0/1)", "Toggles team persisting");
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
            ...
            else while(p.length() < p.maxlen) switch(checktype(getint(p), ci))
            {
                case N_CONNECT:
                {
                    ...
                    if (HSAntiCheat)
                    {
                        loopv (Cheaters)
                        {
                            if (Cheaters[i].IP == getclientip (ci->clientnum))
                            {
                                const char * Name = ci->name;
                                disconnect_client (sender, DISC_IPBAN);
                                sendservmsgf ("\f3%s \f4Client \f0%s disconnected because: \f3Permamently banned for cheating. %s",
                                    MessageDecoration1,
                                    Name,
                                    MessageDecoration2
                                );
                    ...
                    if (HSAntiCheat)
                    {
                        defformatstring (Message)("\f3%s \f6Note: \f0This server is secured by \f3HaythServ-Anti-Cheat \f0- Cheating will be punished with a \f3permament ban. %s",
                            MessageDecoration1,
                            MessageDecoration2
                        );
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_SWITCHNAME:
            {
                ...
                loopv (ReservedNames)
                {
                    if (!strcmp (ci->name, ReservedNames[i].name) && ci->acc->username != ReservedNames[i].acc->username)
                    {
                        Rename (ci);
                        return;
                    }
