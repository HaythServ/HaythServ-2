...
namespace server
{
    ...
    bool setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_MASTER, bool force = false, bool trial = false)
    {
        if(ci->wrong_attempts >= MaxAttempts) {
            if (ci->wrong_attempts == MaxAttempts)
            {
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Maximal amount of fails reached, ignoring /setmaster, #register and #reserve... \f3<~");
                ci->wrong_attempts ++;
            }
            return false;
        }
...
#define EDITHACK 2
            ...
            void BanCheater (clientinfo * ci, const char * Cheat)
            {
                ...
                sendservmsgf ("\f3~> \f4Client \f0%s\f4(\f0%is\f4) has been disconnected for cheating. \f3(%s) \f3<~",
                    Name,
                    Cn,
                    Cheat
                );
        ...
        // >>> Flagruns core
            ...
            void checkflagrun (clientinfo * ci, int flagruntime)
            {
                ...
                else
                {
                    ...
                    sendservmsgf ("\f3~> \f0%s \f4scored the flag in \f0%f \f4seconds \f4[%s] \f3<~",
                        ci->name,
        ...
        // >>> Login system core
            ...
            void Register (clientinfo * ci, const char * username, const char * password)
            {
                if (ci->wrong_attempts >= MaxAttempts)
                {
                    if (ci->wrong_attempts == MaxAttempts)
                    {
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Maximal amount of fails reached, ignoring #register, #reserve and /setmaster... \f3<~");
                        ci->wrong_attempts++;
                    }
                    return;
                }
                if (ci->verified)
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4You have already verified. \f3<~");
                    ci->wrong_attempts++;
                    return;
                }
                ...
                loopv(accounts)
                {
                    ...
                    {
                        defformatstring (msg)("\f3~> \f4An account with the username \"\f0%s\f4\" \f4does already exist. \f3<~", username);
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4We're sorry, but you reached the limit of accounts per IP. \f3<~");
                    ci->wrong_attempts++;
                }
                ...
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Your account has been succsessfully created. \f3<~");
                sendservmsgf("%s verified as '\fs\f5%s\fr'", 
                    colorname (ci),
                    acc.username
                );
            ...
            void Reserve (clientinfo * ci, const char * name)
            {
                if (ci->wrong_attempts >= MaxAttempts)
                {
                    if (ci->wrong_attempts == MaxAttempts)
                    {
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Maximal amount of fails reached, ignoring #reserve, #register and /setmaster... \f3<~");
                        ci->wrong_attempts++;
                    }
                    return;
                }
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4You need to verify to use this command. \f3<~");
                    ci->wrong_attempts++;
                    return;
                }
                if (!strcmp (name, "unnamed"))
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4You can't reserve this name. \f3<~");
                    ci->wrong_attempts++;
                    return;
                }
                int Counter = 0;
                ...
                loopv (reserved_names)
                {
                    ...
                    {
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4This account is already reserved \f3<~");
                        ci->wrong_attempts++;
                        return;
                    }
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4We're sorry - but you reached the maximal amount of reserved names per account. \f3<~");
                    ci->wrong_attempts++;
                    return;
                }
                ...
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4This username has been reserved \f0succsessfully\f4. \f3<~");
            }
            ...
            void ChangePrivs (clientinfo * ci, const char * username, const char * privs)
            {
                ...
                loopv (accounts)
                {
                    if (!strcmp(accounts[i].username, username))
                    {
                        ...
                        {
                            formatstring (Message)("\f3~> \f4Succsessfully changed \f6%s\f4's privilege level to \f6%i\f4. \f3<~",
                                accounts[i].username,
                                newprivs
                            );
                        ...
                        {
                            formatstring (Message)("\f3~> \f4Cannot change \f6%s\f4's privilege level: \f6Old privileges and new privileges are same. \f3<~",
                                accounts[i].username
                            );
                        }
                    }
                ...
                if (!found)
                {
                    formatstring (Message)("\f3~> \f4Cannot change \f6%s\f4's privilege level: \f6No such account. \f3<~",
                        username
                    );
                }
            ...
            void Rename (clientinfo * ci)
            {
                ...
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4You need to verify to use this name, verify now using \f6/setmaster \f0username\f4:\f0password \f3<~");
                ci->wrong_names++;
            }
        ...
        // >>> Commands system core
            ...
            int ParseCommand (clientinfo * ci, const char * Cmd)
            {
                ...
                if (!Current)
                {
                    formatstring (Message) ("\f3~> \f4Unknown command: \f6%s \f3<~", (const char *) Array [0]);
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                    return 2;
                }
                if (Current->RequiredPrivileges > ci->privilege)
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Permission denied. \f3<~");
                    return 3;
                }
            ...
            void Help (clientinfo * ci, const char * Args)
            {
                ...
                loopv (Commands)
                {
                    if (!strcmp (Commands[i].Name, Cmd))
                    {
                        ...
                        {
                            formatstring (Message)("\f3~> \f0Help: \f3Permission denied\f4. \f3<~");
                            sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                            return;
                        }
                    }
                ...
                loopv (Manpages)
                {
                    ...
                    {
                        formatstring (Message)("\f3~> \f0Help: \f3#\f4%s \f0%s\f4: \f0%s \f3<~",
                            Cmd,
                ...
                loopv (Commands)
                {
                    if (!strcmp (Commands[i].Name, Cmd))
                    {
                        formatstring (Message)("\f3~> \f4HELP: \f6No manpage for command %s found. \f3<~", Cmd);
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                        return;
                    }
                }
                formatstring (Message)("\f3~> \f4HELP: \f6Unknown command: %s\f4. \f3<~", Cmd);
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            }
        ...
        // >>> Commands declaration
            ...
            {
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6/setmaster \f0username\f4:\f0password \f3<~");
            }
            ...
            void _REGISTER (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    ...
                    if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                        ...
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#register username password \f3<~");
                }
                else
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#register username password \f3<~");
            ...
            void _SETPRIVS (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    ...
                    if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                    {
                        ...
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#setprivs username newprivs \f3<~");
                }
                else
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#setprivs username newprivs \f3<~");
            ...
            void _GIVEMASTER (clientinfo * ci, const char * Args)
            {
                if (Args && strcmp (Args, "") && Args != NULL)
                {
                    ...
                    for (int i = 0; i < 128; i ++)
                    {
                        if (Array[i] && Array[i] != 0 && Array[i] != NULL)
                        {
                            ...
                            if (!cx)
                            {
                                defformatstring (Message)("\f3~> \f4Unknown client number: \f6%i \f3<~", cn);
                                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                            }
                            else
                            {
                                ...
                                {
                                    defformatstring (Message)("\f3~> \f4Cannot change %s's privileges: \f0Permission denied. \f3<~",
                                        colorname (cx)
                                    );
                                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                                }
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#givemaster cn[,cn2[,...]] \f3<~");
                }
            }
            ...
            void _GIVEADMIN (clientinfo * ci, const char * Args)
            {
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#giveadmin cn[,cn2[,...]] \f3<~");
                }
            }

            void _INFO (clientinfo * ci, const char * Args)
            {
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4HaythServ\f3-v2.0 \f4server mod, \f0based on \f3ORIGINAL SERVER\f4. \f3<~");
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Thanks to \f6/dev/zero \f4for his \f6/setmaster \f0username\f4:\f0password \f4idea. \f3<~");
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Developed by: \f6~Haytham \f4(AKA \f6/dev/tty1 \f4AKA \f6Vice\f4) \f3<~");
                defformatstring (Message)("\f3~> \f4Running on a %s %s \f4machine. \f3<~",
...
# endif
            ...
            void _RENAME (clientinfo * ci, const char * Args)
            {
                ...
                if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                {
                    ...
                    if (!found)
                    {
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Unknown client number. \f3<~");
                        return;
                    }
                    clientinfo * cx = getinfo (cn);
                    if (cx->privilege > ci->privilege)
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#rename <cn> <newname> \f3<~");
                }
            }
            ...
            void _MSG (clientinfo * ci, const char * Args)
            {
                ...
                if (Array[0] && Array[0] != NULL && Array[1] && Array[1] != NULL)
                {
                    ...
                    for (int i = 0; i < 128; i++)
                    {
                        if (cns[i] && cns[i] != 0 && cns[i] != NULL)
                        {
                            ...
                            {
                                defformatstring (Message)("\f3~> \f4PM from \f5%s\f4(\f1%i\f4): \f0%s \f3<~",
                                    ci ? colorname (ci) : "",
                                    ci ? ci->clientnum  : -1,
                                    Array[1]
                                );
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#pm <cn[,cn2[,...]]> <message> \f3<~");
                }
            }
            ...
            void _REVOKEPRIV (clientinfo * ci, const char * Args)
            {
                    ...
                    for (int i = 0; i < 128; i ++)
                    {
                        if (Array[i] && Array[i] != 0 && Array[i] != NULL)
                        {
                            ...
                            else
                            {
                                if (cx->privilege > ci->privilege)
                                {
                                    defformatstring (Message)("\f3~> \f4Cannot revoke %s's privileges: \f0Permission denied. \f3<~",
                                        colorname (cx)
                                    );
                                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                                }
                ...
                {
                    sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f4Usage: \f6#revokepriv cn[,cn2[,...]] \f3<~");
                }
            }
            ...
            void _STATS (clientinfo * ci, const char * Args)
            {
                    ...
                    for (int i = 0; i < 128; i++)
                    {
                        if (Cns [i] && Cns [i] != NULL)
                        {
                            ...
                            else
                            {
                                string Message;
                                string Bak;
                                formatstring (Message)("\f3~> \f0Current game statistics for \f6%s\f4(\f1%i\f4): Frags: \f0%i\f4, deaths: \f0%i\f4 (KPD: \f0%.2f\f4), accuracy: \f0%.2f%%",
                                    cx->name,
                                    cx->clientnum,
                                    cx->state.frags,
                                    cx->state.deaths,
                                ...
                                copystring (Bak, Message);
                                formatstring (Message)("%s \f3<~",
                                    Bak
                                );
                                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
                                if (cx->verified && cx->acc)
                                {
                                    formatstring (Message)("\f3~> \f0Global statistics for \f6%s\f4(\f1%i\f4): Frags: \f0%i\f4, deaths: \f0%i\f4 (KPD: \f0%.2f\f4), accuracy: \f0%.2f%%\f4, team kills: \f0%i\f4, scored: \f0%i\f4, total games: \f0%i",
                                        cx->name,
                                        cx->clientnum,
                                        cx->acc->global_frags,
                ...
                else
                {
                    ...
                    string Message;
                    string Bak;
                    formatstring (Message)("\f3~> \f0Current game statistics for \f6%s\f4(\f1%i\f4): Frags: \f0%i\f4, deaths: \f0%i\f4 (KPD: \f0%f\f4), accuracy: \f0%i%%",
                        ci->name,
                        ci->clientnum,
                        ci->state.frags,
                        ci->state.deaths,
                        ((float) ci->state.frags / max (ci->state.deaths, 1)),
                        (ci->state.damage * 100 / max (ci->state.shotdamage, 1))
                    );
                    copystring (Bak, Message);
                    if (m_teammode)
                    {
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        if(ci && !ci->connected)
        {
            ...
            else while(p.length() < p.maxlen) switch(checktype(getint(p), ci))
            {
                case N_CONNECT:
                {
                    ...
                    if (HSAntiCheat)
                    {
                        loopv (Cheaters)
                        {
                            if (Cheaters[i].IP == getclientip (ci->clientnum))
                            {
                                ...
                                sendservmsgf ("\f3~> \f4Client \f0%s disconnected because: \f3Permamently banned for cheating. <~",
                                    Name
                                );
                                return;
                            }
                    ...
                    if (HSAntiCheat)
                    {
                        sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, "\f3~> \f6Note: \f0This server is secured by \f3HaythServ-Anti-Cheat \f0- Cheating will be punished with a \f3permament ban. <~");
                    }
                    break;
                }
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_SWITCHNAME:
            {
                ...
                loopv (reserved_names)
                {
                    if (!strcmp (ci->name, reserved_names[i].name) && ci->acc != reserved_names[i].acc)
                    {
                        Rename (ci);
                        break;
                    }
