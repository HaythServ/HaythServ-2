...
namespace server
{
    ...
    // >>> HaythServ-v2.0 Core
        ...
        // >>> Accounting system core
            ...
            vector<reserved_name> reserved_names;
                ...
                if(f)
                {
                    ...
                    loopv (accounts)
                    {
                        ...
                        formatstring (Message)("stats %s %u %u %f %f %u %u %i\n",
                            ...
                            accounts[i].global_kpd, accounts[i].global_accuracy, accounts[i].global_teamkills,
                            accounts[i].global_scored, accounts[i].global_matches
                        );
                        f->printf (Message);
                    }
            ...
            void Register (clientinfo * ci, const char * username, const char * password)
            {
                ...
                uint ipaddr = getclientip (ci->clientnum);
                int Counter = 0;
                loopv(accounts)
                {
            ...
            void Reserve (clientinfo * ci, const char * name)
            {
                ...
                loopv (reserved_names)
                {
                    if (!strcmp (reserved_names[i].name, name))
                    {
                ...
                reserved_name & resname = reserved_names.add ();
            ...
            void Rename (clientinfo * ci)
            {
                ...
                defformatstring (Message)("\f3%s \f4You need to verify to use this name, verify now using \f6/setmaster \f0username\f4:\f0password \f3%s",
                    MessageDecoration1,
                    MessageDecoration2
                );
                sendf (ci ? ci->clientnum : -1, 1, "ris", N_SERVMSG, Message);
            ...
            void NewReserve (const char * user, const char * name)
            {
                ...
                loopv (reserved_names)
                {
                    if (reserved_names[i].acc == acc)
                    {
                        Counter++;
                    }
                }
                if (Counter >= MaxReservedNames) { return; }
                reserved_name & resname = reserved_names.add ();
                copystring (resname.name, name);
                resname.acc = acc;
            }

            void NewStats (const char * username, uint frags, uint deaths, double KPD, double Acc, uint TKs, uint Scored, uint Matches)
            {
                account * acc = 0;
                loopv (accounts)
                {
        ...
        if(ci && !ci->connected)
        {
            ...
            else while(p.length() < p.maxlen) switch(checktype(getint(p), ci))
            {
                case N_CONNECT:
                {
                    ...
                    ci->acc = 0;
                    ci->verified = false;
                    // Name protection at CONNECT
                    loopv (reserved_names)
                    {
                        if (!strcmp (text, reserved_names[i].name))
                        {
                            Rename (ci);
                            break;
                        }
