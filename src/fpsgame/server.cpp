...
namespace server
{
    ...
    // >>> HaythServ-v2.0 Core
        ...
        void checkflagrun (clientinfo * ci, int flagruntime)
        {
            if (flagruntime < 500)
            {
                // TODO For HS-2.0.0: Add anticheat.
            }
            else
            {
                bool isbest = false;
        ...
        void Register (clientinfo * ci, const char * username, const char * password)
        {
            ...
            if(f)
            {
                f->printf("// Automatically generated by HaythServ-v2.0 - do not edit, add accounts ingame by #register <username> <password>, change privileges of those as root by #setpriv <username> <privileges>, reserve names ingame by #reserve [username].\n\n");
                loopv (accounts)
                {
                    defformatstring (Message)("account %s %s %i %i\n", 
                ...
                for (int i = 0; i < 128; i++)
                {
                    if (Cns [i] && Cns [i] != NULL)
                    {
                        ...
                        else
                        {
                            string Message;
                            string Bak;
                            formatstring (Message)("\f3~> \f4STATS: \f6%s\f4(\f1%i\f4): Frags: \f0%i\f4, deaths: \f0%i\f4 (KPD: \f0%f\f4), accuracy: \f0%i%%",
                                cx->name,
                                cx->clientnum,
                                cx->state.frags,
                                cx->state.deaths,
